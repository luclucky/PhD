...
cursor.execute("SELECT ST_MetaData(rast) As md FROM stream_network.rlp_stream_rast_testarea;")
...
cursor.execute("SELECT ST_AsGDALRaster(rast, 'GTiff') FROM stream_network.rlp_stream_rast_testarea;")
gdal.FileFromMemBuffer(vsipath, bytes(cursor.fetchone()[0]))
...
nlm_R = ((nlmpy.classifyArray(nlm_R, [.666,.111,.222]) + 1) * 25) + 25
...
nlm_RRC = nlmpy.randomRectangularCluster(int(nROW), int(nCOL), 20, 25)
...
nlm_RE = nlmpy.randomElementNN(int(nROW), int(nCOL), 10000)
...
nlm_RC = nlmpy.randomClusterNN(int(nROW), int(nCOL), .375, n = '8-neighbourhood')
nlm_RC = ((nlmpy.classifyArray(nlm_RC, [.666,.111,.222]) + 1) * 25) + 25
...
cmds = 'raster2pgsql -s 25832 -I -C -M "' + dst_filename + '" -F stream_network.nlmr_testarea | psql -d DB_PhD -h localhost -U lucas '
...
cmds = 'raster2pgsql -s 25832 -I -C -M "' + dst_filename + '" -F stream_network.nlmrrc_testarea | psql -d DB_PhD -h localhost -U lucas '
subprocess.call(cmds, shell=True)
...
cmds = 'raster2pgsql -s 25832 -I -C -M "' + dst_filename + '" -F stream_network.nlmrc_testarea | psql -d DB_PhD -h localhost -U lucas '
subprocess.call(cmds, shell=True)
...
cmds = 'raster2pgsql -s 25832 -I -C -M "' + dst_filename + '" -F stream_network.nlmre_testarea | psql -d DB_PhD -h localhost -U lucas '
subprocess.call(cmds, shell=True)

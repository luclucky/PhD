...
def proCON(confiQ):
    ...
    for habARRA in habARRAs: 
        ...
        for segM in range(25):
            ...
            cursor.execute("""SELECT COUNT(ids) FROM dis_pts_2500_10x10_"""+(habARRA)+""".pts_habitat_red_"""+str(segM+1)+""";""")
            habARE = cursor.fetchall()
            habARE = int(habARE[0][0])
            ...
            for ruN in range(10):
                ...
                for habCONFIq in range(10):
                    ...
                    ICC_NLMR = ICC_bas / ((0.1**2) * habARE)**2
                    ...
                    ICC_NLMRC = ICC_bas / ((0.1**2) * habARE)**2
                    ...
                    ICC_NLMRE = ICC_bas / ((0.1**2) * habARE)**2

                    #####               
                    reSults.append([confiQ, confiQ[15:], habARRA[:-3], habARRA[-2:], 'NLMR', ruN, segM+1,  habCONFIq, ICC_NLMR])
                    reSults.append([confiQ, confiQ[15:], habARRA[:-3], habARRA[-2:], 'NLMRC', ruN, segM+1,  habCONFIq, ICC_NLMRC])
                    reSults.append([confiQ, confiQ[15:], habARRA[:-3], habARRA[-2:], 'NLMRE', ruN, segM+1,  habCONFIq, ICC_NLMRE])

        
    with open("""/home/streib/proCON/ICC_"""+str(confiQ)+"""_NA_tOt.txt""", "w") as output:

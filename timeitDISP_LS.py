...
#     cursor.execute("""ALTER TABLE res_hh.sh_50 add firstcol_"""+str(xxxx+1)+""" bigint, add origin_"""+str(xxxx+1)+""" bigint, add indiv_"""+str(xxxx+1)+""" float, add first20_"""+str(xxxx+1)+""" bigint;""")
...
#     if xxxx == 0:
#         cursor.execute("INSERT INTO res_hh.sh_50 (pts_id, firstcol_"+str(xxxx+1)+", origin_"+str(xxxx+1)+", indiv_"+str(xxxx+1)+", first20_"+str(xxxx+1)+") values "+toins+";") 

#     else:
#         cursor.execute("UPDATE res_hh.sh_50 SET firstcol_"+str(xxxx+1)+" = firstcol_"+str(xxxx+1)+"_arr, origin_"+str(xxxx+1)+" = origin_"+str(xxxx+1)+"_arr, indiv_"+str(xxxx+1)+" = indiv_"+str(xxxx+1)+"_arr, first20_"+str(xxxx+1)+" = first20_"+str(xxxx+1)+"_arr from (values "+toins+") as c(pts_id_arr, firstcol_"+str(xxxx+1)+"_arr, origin_"+str(xxxx+1)+"_arr, indiv_"+str(xxxx+1)+"_arr, first20_"+str(xxxx+1)+"_arr) WHERE pts_id = pts_id_arr;") 
...
if __name__ == '__main__':
    ...
    cursor.execute("SELECT st_extent(geom) FROM calc_hh.pts_habitat_red_50x50_start_0;")
    ...
    cursor.execute("SELECT idS, st_astext(geom) FROM calc_hh.pts_habitat_red_50x50_start_0;")
    ...
    cursor.execute("SELECT start, aim, costs FROM calc_hh.habitats_shortpath_red_nlmr_testarea_50x50_0_start_0_resamp;")
...
#     cursor.execute("""DROP TABLE IF EXISTS res_hh.sh_50;""")
#     cursor.execute("""CREATE table res_hh.sh_50 (pts_id BIGSERIAL);""")
    ...
    for xxxx in range(len(numSH[1:])):
        ...
        t = timeit.Timer("disperSAL_LS(xxxx)", setup="from __main__ import disperSAL_LS, xxxx")
        
        repeat, number = 10, 1
        r = t.repeat(repeat, number) 
        
        if xxxx == 0:
            res_file = open('/home/streib/Schreibtisch/test.txt', 'w')
        else:
            res_file = open('/home/streib/Schreibtisch/test.txt', 'a')

        res_file.write('CONFIQ ' + str(xxxx) + ':' + '\n')

        for l in range(len(r)):
            
            line = str(l+1) +': '+ str(r[l])
